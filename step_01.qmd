...
```{r}
response_levels <- rev(names(daedalus.data::closure_strategy_data))
```
...
```{r echo=FALSE, include=FALSE}
vtnm <- daedalus_country("Vietnam")
sgpr <- daedalus_country("Singapore")

hcap_vtnm <- get_data(vtnm, "hospital_capacity")
hcap_sgpr <- get_data(sgpr, "hospital_capacity")

# responses as time NPIs
responses <- Map(
...
output_vtnm <- run_scenarios(
  vtnm,
  disease_list,
  response = responses,
  time_end = horizon,
  initial_state_manual = list(
    p_infectious = 1 / sum(vtnm$demography),
    p_asymptomatic = 0.99
  )
)
```
...
data_epi_curve <- get_epicurve_data(output_vtnm, disease_tags, format = "long")
...
```{r echo=FALSE, include=FALSE}
data_epi_summary <- get_summary_data(output_vtnm, disease_tags, format = "long")
...
```{r echo=FALSE, include=FALSE}
data_cost_output <- get_cost_data(output_vtnm, disease_tags, format = "long") %>%
...
```{r echo=FALSE, include=FALSE}
data_econ_costs <- get_econ_cost_data(output_vtnm)
...
ggsave(
  ...
  plot_daily_infections_unmit, height = 7, width = 10
)
```
...
ggsave(
  ...
  plot_daily_cases_unmit, height = 7, width = 10
)
```
...
data_epi_curve %>%
  ...
  group_by(response) %>%
  count(
    name = "hcap_vtnm_exceeded_pct"
  ) %>%
  readr::write_csv(
    "tables/table_hcap_vtnm_breaches.csv"
  )
```
