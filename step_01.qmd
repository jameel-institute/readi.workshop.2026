...
severity_profile <- c(0.5, 1.5, 2, 0.5)
...
```{r r0_dist_plot}
# get mean eta, not per age group
eta_means <- vapply(disease_list, function(x) mean(x$eta), FUN.VALUE = numeric(1))
p_eta <- ggplot() +
  stat_histinterval(
    aes(
      eta_samples,
...
```{r}
day_now <- 30
horizon <- 100 # works better for Vietnam
```

### Run Phase 1 scenarios

```{r echo=FALSE, include=FALSE}
country <- daedalus_country("Vietnam")
...
output <- run_scenarios(
  ...
  initial_state_manual = list(
    p_infectious = 1 / sum(country$demography),
    p_asymptomatic = 0.99
  )
)
```
    ...
    if (m %in% c("total_hosp", "daily_hospitalisations")) {
      plot_epi_curves_unmit <- plot_epi_curves_unmit +
        ...
        annotate(
          ...
          x = 50, y = hcap,
          label = "Hosp. cap.",
          colour = "red", hjust = "inward", vjust = "outward",
          fontface = "bold"
        )
    }
...
plot_unmit_epi_summary <-
  ...
  labs(
    x = NULL,
    y = glue::glue("Count (up to day {horizon})"),
    colour = "Proj. interv.",
    caption = "Please consider that there is under-ascertainment.",
  ) +
  guides(
...
plot_unmit_deaths_by_age <-
  ...
  labs(
    y = glue::glue("Cumulative deaths (up to day {horizon})"),
    x = "Age group",
    colour = "Proj. interv.",
    caption = "Please consider that there is under-ascertainment."
  ) +
...
plot_unmit_gva_loss <-
  ...
  scale_y_continuous(
    labels = scales::label_currency(
      prefix = "$", suffix = " B", scale = 1e-3, accuracy = 1
    )
  ) +
  scale_colour_viridis_d(
    begin = 0.1, direction = -1,
...
data_epi_curve %>%
  ...
  group_by(response) %>%
  summarise(
    hcap_exceeded_pct = sum(max(value) > hcap)
  ) %>%
  readr::write_csv(
    "tables/table_hcap_breaches.csv"
  )
```
